...
/*
//cursos
if($cursos != "" && $cursos != "Ninguno")
$cursos_juntos = $cursos_juntos."<br />".$cursos;
...
$t->set_var("idiomas",$mis_idiomas." ".$mis_idiomas_dos);
$t->set_var("cursos",$mis_cursos);
$t->set_var("congresos",$mis_congresos);
...
if($sql_post == "")
	$sql = "SELECT * FROM empleos, ciudades, empresas WHERE empleos.ciudad = ciudades.numero_ciudad AND empresas.id_empresa = empleos.id_empresa  order by empleos.tipo asc, empleos.activo desc, empleos.publicacion desc, rand() ";
else
	...
	$rpp = 30; // cantidad de empleos q quiero ver
...
while($ofertas = mysql_fetch_array($result2))
{
	...
	$t->set_var("dispo", $nombre_disponibilidad["nombre_disponibilidad"]);
		...
		//detalle nuevo
		$enlace = "<a href='{empleo_potulado}". str_replace(" ","-",utf8_decode($ofertas['titulo']))."-en-".str_replace(" ","-",$ofertas['nombre_ciudad'])."/".$ofertas['id_empleo']."' title='Ver Mas'>"; // para el enlace de la home
...
//PAGINACION
	...
	{ 
		...
		{ 
			...
			if( isset($_GET['ciudad']) )
				$ciudad_bus = $_GET['ciudad'];
			else
				$ciudad_bus = $_POST['ciudad'];	
			if(isset($_GET['areas']))
				$area_bus = $_GET['areas'];
			else
				$area_bus = $_POST['areas'];
			...
			elseif($_GET['action'] == "empleos")
				...
				$paginacion .= "<div class='paginacion'><a href='/?action=buscar_empleo&page=$i&ciudad=".$ciudad_bus."&area=".$area_bus."'>$i</a></div> ";			
		} 
		else 
		{ 
			if($_GET['action'] == "mostrar_home_page" || $_GET['action'] == "")
			...
			elseif($_GET['action'] == "empleos")	
				...
				$paginacion .= "<div class='selected'><a href='/?action=buscar_empleo&page=$i&ciudad=".$ciudad_bus."&area=".$area_bus."'>$i</a></div> ";	
	...
	/*$pathXML = "/mounted-storage/home129/sub032/sc75253-XNXJ/www.jobtime.com.ar/rodrigo_sitemap.xml";
		
	$fileXML = $pathSite.$pathXML;
	
	$fhXML = fopen($fileXML, 'w+');
		
	fwrite($fhXML, $smap->generate() );
...
$t->set_var("detalle_corto", substr(utf8_decode($dealle_ofer), 0, 120));
...
$sql = "SELECT * FROM empleos, ciudades, empresas WHERE empleos.ciudad = ciudades.numero_ciudad AND empresas.id_empresa = empleos.id_empresa ";
