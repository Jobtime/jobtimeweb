...
if($sql_post == "")
	$sql = "SELECT * FROM empleos, ciudades, empresas WHERE empleos.ciudad = ciudades.numero_ciudad AND empresas.id_empresa = empleos.id_empresa AND empleos.activo = 1 order by empleos.tipo asc, empleos.publicacion desc";
else
	...
	$rpp = 20; // cantidad de empleos q quiero ver
...
while($ofertas = mysql_fetch_array($result2))
{
	...
	$t->set_var("dispo", $nombre_disponibilidad["nombre_disponibilidad"]);
		...
		//$smap->add("http://www.jobtime.com.ar/postularme-".str_replace(" ","-",utf8_decode($ofertas['titulo']))."-en-".str_replace(" ","-",$ofertas['nombre_ciudad'])."/".$ofertas['id_empleo'],$dia );
