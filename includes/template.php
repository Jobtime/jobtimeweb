...
// PARCHEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEE
    function set_text($varname, $value = "")
    {
        ...
        if (!is_array($varname))
        {
            if (!empty($varname))
            {
                ...
                $this->textkeys[$varname] = "/".$this->textname($varname)."/s";
     ...
     */
    function parse($target, $handle, $append ='x')
    {
        if (!is_array($handle))
        {
            $str = $this->subst($handle);
            //  echo htmlentities($str) . "<br><br><br><br>";
            if ($append)
            {
    ...
    function finishText($str) {
        switch ($this->unknowns) {
            ...
            case "remove":
                $str = preg_replace('/\\[{([0-9]+)\|([0-9A-Za-z|�|�|�|�|�|�|�|�|�|�])([^}]*)([0-9A-Za-z|�|�|�|�|�|�|�|�|�|�])}\]/', "", $str);
                //if(strpos($str, "'gtm':'1',")===false) $str = preg_replace('/\\[{([0-9]+)\|([0-9A-Za-z|�|�|�|�|�|�|�|�|�|�])([^}]*)([0-9A-Za-z|�|�|�|�|�|�|�|�|�|�])}\]/', "", $str);
                break;

            case "comment":
                $str = preg_replace('/\\[{([0-9]+)\|([0-9A-Za-z|�|�|�|�|�|�|�|�|�|�])([^}]*)([0-9A-Za-z|�|�|�|�|�|�|�|�|�|�])}\]/', "<!-- Template $handle: Variable \\1 undefined -->", $str);
                break;
        }
    ...
    */
    ...
    function translateHTM2($pathLang, $idioma, $template, $handle)
    {
        ...
        foreach (array_keys($textos) as $key)
        {
            $key2 = $key ."|". $textos[$key];
            if ($traduc[$key] != "")
                $this->set_text($key, stripslashes($traduc[$key]));
            else
                $this->set_text($key, stripslashes($traduc_gral[$key]));
        }
    }
    ...
    function leerXML($xmlFile, $idioma, $pathLang)
    {
        ...
        if (!$dom)
            ...
            foreach($vars as $var)
            {
                ...
                $return[$content_id] = $content_name;
