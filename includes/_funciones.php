...
include("captcha/PHPCaptcha.lib.php");

	if(!(Captcha::verificaCaptcha($_REQUEST['captcha'])))
	{

		session_register("valuesForm");
		$_SESSION['valuesForm'] = array('razon' =>  $_POST['razon_social'],
		'tel' => $_POST['Telefono'],
		'mail' => $_POST['email'],
		'cuil' => $_POST['cuit']);
...
while ($estudios = mysql_fetch_array($result)) 
{
	...
	{
		$langs = $langs." ".$lang['nombre']." Nivel: ".$lang['nivel'];
	}
	
	$t->set_var("idiomas_parse",$langs);	

/*if(strlen($mis_idiomas) > 60)
{
	$resto_idioma = substr ($mis_idiomas, 0, 58); 
	$enlace_idiomas = "<a href='#' title='".$mis_idiomas.", ".$mis_idiomas_dos."'>".$resto_idioma."...</a>";	
}
else
	$enlace_idiomas = $mis_idiomas." ".$mis_idiomas_dos;
*/
if(strlen($mis_cursos) > 60)
{
	$resto_curso = substr ($mis_cursos, 0, 58); 
	$enlace_cursos = "<a href='#' title='".$mis_cursos."'>".$resto_curso."...</a>";	
}
else
	$enlace_cursos = $mis_cursos;

if(strlen($mis_congresos) > 60)
{
	$resto_congreso = substr ($mis_congresos, 0, 58); 
...
function editar_curso($dni)
{
	...
	$consulta = "select cursos, dni from estudios where dni = ".$dni;
	
	$result = mysql_query($consulta);
	
	while($estudios = mysql_fetch_array($result))
		 {
			 $t->set_var("titulo_curso",$estudios['cursos']);
			 $t->set_var("numero_estudio",$id_estudio);
...
function editar_idioma($dni)
{
	...
	$consulta = "select nombre, nivel, dni from idiomas where dni = ".$dni;
...
function editar_congresos($dni)
{
	...
	$consulta = "select congresos, dni from estudios where dni = ".$dni;
	
	$result = mysql_query($consulta);
	
	while($estudios = mysql_fetch_array($result))
		 {
			 if($estudios['congresos'] != "")
			 {
				 $t->set_var("titulo_congreso",$estudios['congresos']);
				 $t->set_var("numero_estudio",$id_estudio);
...
$sql = "INSERT INTO postulaciones(id_empleo, usuario, fecha, activo, interes)  VALUES ($empleo, $usuario, '".$fecha_actual."',1,2)";
