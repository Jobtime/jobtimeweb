...
class PHPPaging {
	...
	*	@var string
	**/
	var $mostrarActual = "<li><a href='#' title='Pagina Actual' class='current-page'>{n}</a></li>";
	...
	*/
	function ejecutar ($db) {
		...
		if(!empty($this->sql)) {
			$result = (isset($this->conn)) ? @mysqli_query($db, $this->sql) : @mysqli_query($db, $this->sql);
		...
		if($this->numTotalRegistros >= 1) {
			...
			if(!empty($this->sql)) {
				...
				$result = (isset($this->conn)) ? @mysqli_query($db, $sql) : @mysqli_query($db,$sql);
